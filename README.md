...
工具是否值得进入或继续扩展，统一按 [`TOOL_POLICY.md`](./TOOL_POLICY.md) 的“真实工作流 / 重复痛点 / 可复用性 / 杠杆价值”四项标准审核。
...
2. **真实需求优先**：工具和显著新功能必须能对应真实工作流、重复痛点和明确收益；不为了扩充工具数量而建设。
3. **接口稳定**：已有 CLI、输入字段和默认输出语义保持向后兼容；确需破坏性修改时升级版本并明确记录。
4. **配置优先**：时间窗、坐标范围、站点名、变量标签、QC 规则等写入配置，不写死在核心代码。
5. **跨环境优先**：Python 工具统一使用 `pathlib`，不依赖固定盘符或工作目录；确需操作系统专用封装时明确平台边界，并继续避免写死绝对路径。
6. **低依赖**：能用标准库或现有依赖解决的任务，不额外引入包。
7. **可检查**：每个工具保留最小 smoke test 或明确的验证记录；错误应显式报出，不静默生成错误结果。
8. **不臃肿**：不同数据组织模式可分脚本，但不为少量逻辑无故拆分模块；预想中的未来需求不作为增加抽象层的充分理由。
9. **AI 与确定性程序分工**：AI 负责理解和整理语义，用户负责关键裁决；文件复制、校验、hash、打包等机械操作交给脚本。

## AI / 人工维护约束

- 修改前先阅读本文件、[`TOOL_POLICY.md`](./TOOL_POLICY.md) 和目标工具的 `README.md`。
- 新增工具或显著扩展前，先完成 TOOL_POLICY 的“工具准入四问”；不足 3/4 时默认先保持为项目内脚本或临时实现。
